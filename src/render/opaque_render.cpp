...
namespace vke_render
{
    ...
    {
        swapChainFramebuffers.resize(environment->swapChainImageViews.size());
        for (size_t i = 0; i < environment->swapChainImageViews.size(); i++)
        {
            ...
            if (vkCreateFramebuffer(environment->logicalDevice, &framebufferInfo, nullptr, &swapChainFramebuffers[i]) != VK_SUCCESS)
            {
    ...
    void OpaqueRenderer::recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex)
    {
    ...
    void OpaqueRenderer::drawFrame()
    {
        ...
        vkResetCommandBuffer(environment->commandBuffers[currentFrame], 0);
        recordCommandBuffer(environment->commandBuffers[currentFrame], imageIndex);
