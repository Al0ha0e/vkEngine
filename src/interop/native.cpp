...
namespace vke_interop
{
    ...
    {
        vke_common::Scene *scene = GetCurrentScene();
        scene->SetLocalPosition(GetEntity(scene, id), ToGlm(*position));
    }
    ...
    {
        vke_common::Scene *scene = GetCurrentScene();
        scene->SetLocalRotation(GetEntity(scene, id), ToGlm(*rotation));
    }
