...
{
    static vke_component::AudioSource *GetAudioSource(uint32_t entity)
    {
    ...
    static vke_component::AudioListener *GetAudioListener(uint32_t entity)
    {
        auto *scene = vke_common::SceneManager::GetInstance();
        entt::entity ent = static_cast<entt::entity>(entity);
    ...
    {
        auto *src = GetAudioSource(entity);
        if (src)
            src->Play();
    }
    ...
    {
        auto *lis = GetAudioListener(entity);
        if (lis)
