...
class Param:
    ...
    def loadFromJSON(self, settings):
        ...
        self.SunLightColor = glm.vec3(
            settings["SunLightColor"][0],
            settings["SunLightColor"][1],
            settings["SunLightColor"][2],
        )
...
def IntegralMultiScattering(param, samplePoint, lightDir, _transmittanceLut):
    ...
    for i in range(0, N_DIRECTION):
        ...
        dis = RayIntersectSphere(
            glm.vec3(0, 0, 0),
            param.PlanetRadius + param.AtmosphereHeight,
            samplePoint,
            viewDir,
        )
        d = RayIntersectSphere(
            glm.vec3(0, 0, 0), param.PlanetRadius, samplePoint, viewDir
        )
        if d > 0:
            dis = min(dis, d)
        ...
        for j in range(0, N_SAMPLE):
            ...
            if h < 0:
                ...
                # p += viewDir * ds
                # continue
            sigma_s = RayleighCoefficient(param, h) + MieCoefficient(
                param, h
            )  # scattering
...
def GenTransmittanceLUT(param, path):
    ...
    for i in range(0, 256):
        for j in range(0, 64):
            ...
            uv = glm.vec2((i + 0.5) / 256, (j + 0.5) / 64)
            ...
            pixels[i, j] = (int(color.x * 255), int(color.y * 255), int(color.z * 255))
...
def GenMultiScatteringLUT(param, path, transmittance_LUT):
    ...
    for i in range(0, 32):
        for j in range(0, 32):
            print(i, j)
            uv = glm.vec2((i + 0.5) / 32.0, (j + 0.5) / 32.0)
            mu_s = uv.x * 2.0 - 1.0
            ...
            pixels[i, j] = (
                glm.clamp(int(filterNan(color.x) * 255), 0, 255),
                glm.clamp(int(filterNan(color.y) * 255), 0, 255),
                glm.clamp(int(filterNan(color.z) * 255), 0, 255),
            )
